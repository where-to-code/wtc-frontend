...
describe('Registartion tests', () => {
    ...
    it('render the loader while loading', () => {
        ...
        const { getByText } = renderWithRedux(<Registration />, { store });
    ...
    it('push to / if logo is clicked', () => {
        ...
        const { getByAltText, container } = renderWithRedux(<TestingRouter ComponentWithRedirection={props => <Registration {...props} />} RedirectUrl={redirectUrl} MainRoute={mainRoute} />, {}, { route: mainRoute })
        fireEvent.click(getByAltText('logo'))
        expect(container.innerHTML).toEqual(
            expect.stringContaining(redirectUrl)
        )
    })
    it('push to / if signup is clicked and inputs pass validation', () => {
        const redirectUrl = '/'
        const mainRoute = '/signup'
        const { getByPlaceholderText, getByText, container } = renderWithRedux(<TestingRouter ComponentWithRedirection={props => <Registration {...props} />} RedirectUrl={redirectUrl} MainRoute={mainRoute} />, {}, { route: mainRoute })
    ...
    it("does anything if signup is clicked and inputs don't pass validation", () => {
        const redirectUrl = '/'
        const mainRoute = '/signup'
        const { getByPlaceholderText, getByDisplayValue, getByText } = renderWithRedux(<TestingRouter ComponentWithRedirection={props => <Registration {...props} />} RedirectUrl={redirectUrl} MainRoute={mainRoute} />, {}, { route: mainRoute })
        fireEvent.change(getByPlaceholderText('First Name'), { target: { value: 'giacomo' } })
        fireEvent.change(getByPlaceholderText('Last Name'), { target: { value: 'benati' } })
        fireEvent.change(getByPlaceholderText('Email'), { target: { value: 'giacomobenati@mailbox.org' } })
        fireEvent.change(getByPlaceholderText('Password'), { target: { value: 'giacomo1' } })
    ...
    describe("display an error message if validation don't pass", () => {
        it('test first name validation', () => {
            const { getByPlaceholderText, getByText } = renderWithRedux(<Registration />);
